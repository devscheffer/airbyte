...
class MarvelApiStream(HttpStream, ABC):
    ...
    def request_params(
        ...
        api_pub_key = self.pub_key
        api_priv_key = self.priv_key
        timestamp = self.timestamp

        api_hash = hash_params(
            api_pub_key,
            api_priv_key,
            timestamp,
        )
...
class IncrementalMarvelApiStream(MarvelApiStream, ABC):
    """
...
class Employees(IncrementalMarvelApiStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
...
class SourceMarvelApi(AbstractSource):
    def check_connection(
        ...
        api_hash = hash_params(api_key, config["priv_key"], timestamp, )
